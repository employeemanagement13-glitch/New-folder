// Components/hr/announcements/AddNotificationModal.tsx (Updated)
'use client';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const notificationData = {
        ...formData,
        created_by: userRole, // Use the role passed from parent
        ...
        // Set scheduled_for to current time if empty and status is sent
        // scheduled_for: formData.scheduled_for || new Date().toISOString(),
        // Set status to sent if scheduled_for is in the past or empty
      };
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#333333]">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              Create and schedule company-wide or department-specific announcements
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-[#1a1a1a] rounded-xl p-4 border border-[#333333]">
            <div className="text-sm text-gray-400">
              This announcement will be created by: <span className="text-[#ff9d00] font-medium">{userRole}</span>
            </div>
          </div>
